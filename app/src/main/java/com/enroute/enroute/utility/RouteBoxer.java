...
//	protected final Logger logger = LoggerFactory.getLogger(this.getClass());
	...
	private void buildGrid_(List<LatLng> vertices, double range) {
		...
		Log.d("RouteBoxer",
				"vertices[0]"+vertices.get(0).toString()+" vertices["+(vertices.size()-1)+"] "+vertices.get(vertices.size()-1).toString());
		...
		Log.d("RouteBoxer",
				"pass1 latGrid size"+latGrid_.size());
		...
		for (int i1 = 1; this.latGrid_.get(1) > routeBounds.getSouthWest().lat(); i1++) {
			...
			if (i1 % 1000 == 0) {
				Log.d("RouteBoxer", "pass2 loop: " + String.valueOf(i1));
			}
		}
		Log.d("RouteBoxer",
				"pass2 latGrid size"+latGrid_.size());
		...
		Log.d("RouteBoxer",
				"pass1 lngGrid_ size"+lngGrid_.size());
		...
		Log.d("RouteBoxer",
				"pass2 lngGrid_ size"+lngGrid_.size());
